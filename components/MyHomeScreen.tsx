  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-800 p-6 md:p-8 rounded-xl shadow-2xl flex flex-col text-slate-100">
      <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-700">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-lime-500 flex items-center">
      ...
      <div className={`p-6 md:p-8 my-4 rounded-lg shadow-xl text-center ${currentHouseVisual.bgColor} ${currentHouseVisual.textColor} transition-all duration-500 ease-in-out transform hover:scale-105`}>
        ...
        <h2 className="text-2xl md:text-3xl font-bold mb-1">{UI_TEXT_TH[currentHouseVisual.nameKey]}</h2>
        <p className="text-md opacity-90 mb-3">{currentHouseVisual.description}</p>
        <p className="text-sm font-medium bg-black/20 px-3 py-1 rounded-full inline-block">{UI_TEXT_TH.currentHouseLevelLabel} {currentHouseLevel}</p>
      </div>

      {/* Practice Nook Section */}
      <div className="bg-slate-700/50 p-4 md:p-6 rounded-lg shadow-md mb-6">
        <h3 className="text-lg font-semibold mb-2 text-purple-300 flex items-center">
          <MusicPracticeIcon className="w-5 h-5 mr-2" /> {UI_TEXT_TH.practiceNookTitle}
        </h3>
        <p className="text-xs text-slate-300 mb-3">{UI_TEXT_TH.practiceNookDescription}</p>
        <button
      ...
      <div className="bg-slate-700/50 p-4 md:p-6 rounded-lg shadow-md mb-6">
        <h3 className="text-lg font-semibold mb-3 text-lime-300 flex items-center">
            <GiftIcon className="w-5 h-5 mr-2"/> {UI_TEXT_TH.houseBenefitDescription}
        </h3>
        <ul className="space-y-1 text-sm text-slate-200 list-disc list-inside pl-2">
            ...
            {currentBenefits.dailyLoginBonusGCoins === 0 && currentBenefits.trainingXpMultiplier === 1.00 && currentBenefits.trainingGCoinMultiplier === 1.00 && (
                <li className="text-slate-400">ยังไม่มีสิทธิประโยชน์จากระดับบ้านปัจจุบัน</li>
            )}
        </ul>
      </div>
      ...
      <div className="bg-slate-700/50 p-4 md:p-6 rounded-lg shadow-md mb-6">
        ...
        {playerData.ownedFurnitureIds && playerData.ownedFurnitureIds.length > 0 ? (
            ...
            {playerData.ownedFurnitureIds.map(furnitureId => {
              ...
              return (
                <div key={furnitureId} className="bg-slate-600/70 p-3 rounded-md flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-medium text-emerald-200">{UI_TEXT_TH[details.nameKey]}</p>
                    <p className="text-xs text-slate-300">{UI_TEXT_TH[details.effectDescriptionKey]}</p>
                  </div>
                </div>
              );
            })}
          ...
          <p className="text-slate-400 text-sm">{UI_TEXT_TH.noFurnitureOwned}</p>
        )}
      </div>
      ...
      {canUpgrade ? (
        <div className="bg-slate-700 p-6 rounded-lg shadow-md">
          ...
          <div className="flex items-center justify-between mb-4">
            <span className="text-lg text-slate-300">{UI_TEXT_TH.costLabel}</span>
          ...
          {!canAffordUpgrade && (
            <p className="text-red-400 text-sm text-center mt-3">{UI_TEXT_TH.notEnoughGCoinsForUpgrade}</p>
          )}
        </div>
      ) : (
        <p className="text-xl text-center font-semibold text-green-400 p-6 bg-slate-700/80 rounded-lg shadow-md">
