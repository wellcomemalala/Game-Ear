...
import { ACTUAL_PIANO_LOWEST_MIDI_NOTE, ACTUAL_PIANO_HIGHEST_MIDI_NOTE, midiNoteToName } from '../constants';
...
const PianoKeyboard: React.FC<PianoKeyboardProps> = ({ highlightedNotes, onNoteClick, disabled = false }) => {
  ...
  let whiteKeyIndex = 0;
  const totalWhiteKeys = Array.from({ length: ACTUAL_PIANO_HIGHEST_MIDI_NOTE - ACTUAL_PIANO_LOWEST_MIDI_NOTE + 1 }, (_, i) => !isBlackKey(ACTUAL_PIANO_LOWEST_MIDI_NOTE + i)).filter(Boolean).length;
  const whiteKeyWidthPercent = 100 / totalWhiteKeys;


  for (let midiNote = ACTUAL_PIANO_LOWEST_MIDI_NOTE; midiNote <= ACTUAL_PIANO_HIGHEST_MIDI_NOTE; midiNote++) {
  ...
  return (
    <div className="w-full h-32 md:h-40 flex justify-start items-stretch bg-slate-700 p-2 rounded-lg shadow-inner select-none my-4 relative overflow-x-auto min-w-[320px] md:min-w-[500px]">
