...
import {
  ...
  Difficulty, GameMode, InstrumentSoundId, NotificationMessage, UnlockedItemType, ThaiUIText
} from '../types';
import {
...
interface MonsterBattlePageProps {
  ...
  onBattleEnd: () => void; // Called after win or loss acknowledgement
...
const MonsterBattlePage: React.FC<MonsterBattlePageProps> = ({
  ...
  useEffect(() => {
    ...
    if (definition) {
      ...
      onBattleEnd(); // Should not happen if flow is correct
    }
  ...
  const generateStageQuestion = useCallback(() => {
    ...
    if (itemBank.length === 0) { // Fallback if specified difficulty yields no items
    ...
    setCurrentQuestion({
      item: questionItem,
      rootFrequency,
      options: options as (IntervalInfo[] | ChordInfo[]), // Type assertion
  ...
  const playCurrentSound = useCallback(() => {
    ...
    if (currentQuestion.item.hasOwnProperty('semitones') && !Array.isArray((currentQuestion.item as IntervalInfo).semitones)) { // Interval
  ...
  const handleAnswer = (answerId: string) => {
    ...
    if (!correct) {
      ...
      // No XP/Coins for monster battles, only memento on win
    }
    // If correct, next action will be handled by button for "Next Stage" or "Claim Victory"
  };

  const handleNextAction = () => {
    if (battleOutcome) { // If battle ended (win/loss), button goes to lair
      onBattleEnd();
      return;
    }
    ...
    if (isCorrect) {
      if (currentStageIndex + 1 < (monsterDef?.stages.length || 0)) {
        ...
        // generateStageQuestion will be called by useEffect
      } else {
        // All stages completed correctly
        setBattleOutcome('victory');
        if (monsterDef) {
            recordMonsterDefeat(monsterDef.id);
        }
      }
    }
    // If incorrect, battleOutcome is 'defeat', button already leads to onBattleEnd
  };
    ...
    {battleOutcome === 'victory' && (
        <div className="flex-grow flex flex-col items-center justify-center space-y-4 text-center">
            ...
            <button onClick={handleNextAction} className="btn-primary mt-4 px-8 py-3 text-lg">
                {UI_TEXT_TH.backToMenu} {/* Or "To Monster Lair" */}
            </button>
        </div>
    )}
    ...
    {!battleOutcome && currentQuestion && (
        <>
          ...
          <div className="h-28 flex flex-col justify-end items-center space-y-4 pt-4 border-t border-slate-700">
            {showFeedback && typeof isCorrect === 'boolean' && !isCorrect && ( // Only show feedback message if incorrect, as battle ends
            ...
            {showFeedback && isCorrect && (
                ...
                >
                    {currentStageIndex + 1 < monsterDef.stages.length ? UI_TEXT_TH.nextQuestion : UI_TEXT_TH.claimRewardButton /* Actually will show victory message first */} &rarr;
                </button>
            )}
          </div>
        </>
