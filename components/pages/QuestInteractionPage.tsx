...
import { PlayerData, NPCId, QuestId, QuestStatusEnum, ThaiUIText, QuestDefinition, QuestObjective, QuestObjectiveType, QuestItemId } from '../../types'; // Changed QuestStatus to QuestStatusEnum
...
const QuestInteractionPage: React.FC<QuestInteractionPageProps> = ({
  ...
  const handleQuestProgressWithItem = (questId: QuestId, objectiveIndex: number, itemId: string | QuestItemId) => {
    onProgressQuest(questId, objectiveIndex, { itemId });
  };
                    ...
                    if (currentObjDef && currentObjDef.type === QuestObjectiveType.COLLECT_ITEM && currentObjDef.targetId) {
                        ...
                        if (itemInInventory && itemInInventory.quantity >= (currentObjDef.targetCount || 1)) {
                            ...
                            if(nextObjectiveDef && nextObjectiveDef.type === QuestObjectiveType.TALK_TO_NPC && nextObjectiveDef.targetId === npcId) {
                                return (
                                    <button
                                        onClick={() => onProgressQuest(displayQuest!.id, nextObjectiveIndex, { itemId: currentObjDef.targetId })}
