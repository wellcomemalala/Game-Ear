...
interface MissionsPageProps {
  ...
  claimMissionReward: (missionId: MissionId) => boolean; // This is missionSystem.claimMissionReward
  onBackToMenu: () => void;
}
...
const MissionRewardIcon: React.FC<{ type: MissionRewardType, className?: string }> = ({ type, className = "w-4 h-4" }) => {
  switch (type) {
    ...
    case MissionRewardType.PET_XP:
      return <PetIcon className={`${className} text-pink-400`} />; 
    default:
      return null;
  }
};
...
const MissionsPage: React.FC<MissionsPageProps> = ({
  playerData,
  claimMissionReward, // This is missionSystem.claimMissionReward
  onBackToMenu,
}) => {
