...
import { PlayerData, ThaiUIText, NotificationMessage, AppView, FurnitureId, PracticeNookResult, NPCId, ChildGrowthStage, ChildNeedType, ShopItemId, FamilyActivityId } from '../../types';
import {
    ...
    SIDE_JOB_MIN_FAMILY_HAPPINESS_THRESHOLD, 
    MAX_CHILD_HAPPINESS, MAX_MARRIAGE_HAPPINESS,
    BIRTHDAY_CAKE_COST, // Added
} from '../../constants';
...
import { CakeIcon } from '../icons/CakeIcon'; // Added CakeIcon
...
// ... (HouseVisualIcon, FurnitureDisplayIcon, ChildVisualIcon, houseVisuals definitions remain the same)
...
const ChildVisualIcon: React.FC<{ stage: ChildGrowthStage | undefined, className?: string, isCrying?: boolean, isSick?: boolean, isSleeping?: boolean }> = ({ stage, className, isCrying, isSick, isSleeping }) => {
    ...
    switch(stage) {
        ...
        case ChildGrowthStage.SCHOOL_AGE: return <span className={`${className} text-3xl text-blue-300`} role="img" aria-label="เด็กวัยเรียน">🧑‍🎓</span>;
...
export const MyHomeScreen: React.FC<MyHomeScreenProps> = ({ 
  playerData,
  upgradeHouse,
  ...
  celebrateBirthday, // Destructured
}) => {
  ...
  const [parkTripCooldownDisplay, setParkTripCooldownDisplay] = useState<string | null>(getFamilyActivityCooldownTime(FamilyActivityId.PARK_TRIP)); 
  const [isChildBirthdayToday, setIsChildBirthdayToday] = useState(false);
  const [isSpouseBirthdayToday, setIsSpouseBirthdayToday] = useState(false);
  ...
  useEffect(() => {
    ...
    if (playerData.childData?.birthday) {
      setIsChildBirthdayToday(playerData.childData.birthday.month === currentMonth && playerData.childData.birthday.day === currentDay);
    } else {
      setIsChildBirthdayToday(false);
    }

    if (playerData.spouseId) {
      const spouseDef = getNPCDefinition(playerData.spouseId);
      if (spouseDef?.birthday) {
        setIsSpouseBirthdayToday(spouseDef.birthday.month === currentMonth && spouseDef.birthday.day === currentDay);
      } else {
        setIsSpouseBirthdayToday(false);
      }
    } else {
      setIsSpouseBirthdayToday(false);
    }
  }, [playerData.childData, playerData.spouseId]);
  ...
  useEffect(() => { 
    const updateParkTripDisplay = () => setParkTripCooldownDisplay(getFamilyActivityCooldownTime(FamilyActivityId.PARK_TRIP));
    updateParkTripDisplay();
    const interval = setInterval(updateParkTripDisplay, 1000);
    return () => clearInterval(interval);
  }, [getFamilyActivityCooldownTime]);
  ...
  const handlePerformBusking = () => {
    performBusking();
    setBuskingCooldownDisplay(getBuskingCooldownTime());
  };

  const handlePerformParkTrip = () => { 
    performFamilyActivity(FamilyActivityId.PARK_TRIP);
    setParkTripCooldownDisplay(getFamilyActivityCooldownTime(FamilyActivityId.PARK_TRIP));
  };

  const handleCelebrateBirthday = (target: 'child' | 'spouse') => {
    celebrateBirthday(target);
  };
  ...
  const isChildSleeping = child?.sleepiness === 0 && (Date.now() - (child.lastSleptTimestamp || 0)) < 1000 * 60 * 5; 
  ...
  const canDoFamilyActivity = playerData.isMarried && playerData.childData;
  
  const canPerformSideJob = !( (playerData.isMarried && playerData.marriageHappiness < SIDE_JOB_MIN_FAMILY_HAPPINESS_THRESHOLD) ||
                              (playerData.childData && playerData.childData.happiness < SIDE_JOB_MIN_FAMILY_HAPPINESS_THRESHOLD) );

  const getAcademicPerformanceColor = (performance: string): string => {
    if (performance === 'A') return 'text-green-400';
    if (performance === 'B') return 'text-lime-400';
    if (performance === 'C') return 'text-yellow-400';
    if (performance === 'D') return 'text-orange-400';
    if (performance === 'F') return 'text-red-400';
    return 'text-white';
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-800 p-6 md:p-8 rounded-xl shadow-2xl flex flex-col text-slate-100">
      ...
      {isChildBirthdayToday && child && (
        <div className="bg-yellow-500/30 p-4 rounded-lg shadow-md mb-6 text-center">
          <p className="text-lg font-semibold text-yellow-200 text-outline-black">🎉 {UI_TEXT_TH.childBirthdayMessage.replace('{childName}', child.name)} 🎉</p>
          <button
            onClick={() => handleCelebrateBirthday('child')}
            className="mt-2 btn-primary bg-yellow-500 hover:bg-yellow-600 focus:ring-yellow-400 disabled:opacity-60"
            disabled={playerData.gCoins < BIRTHDAY_CAKE_COST}
          >
            <CakeIcon className="w-5 h-5 mr-2" /> {UI_TEXT_TH.buyBirthdayCakeButtonLabel.replace('{cost}', BIRTHDAY_CAKE_COST.toString())}
          </button>
        </div>
      )}
      {isSpouseBirthdayToday && playerData.spouseId && (
        <div className="bg-pink-500/30 p-4 rounded-lg shadow-md mb-6 text-center">
          <p className="text-lg font-semibold text-pink-200 text-outline-black">🎂 {UI_TEXT_TH.spouseBirthdayMessage.replace('{spouseName}', getNPCName(playerData.spouseId, UI_TEXT_TH))} 🎂</p>
          <button
            onClick={() => handleCelebrateBirthday('spouse')}
            className="mt-2 btn-primary bg-pink-500 hover:bg-pink-600 focus:ring-pink-400 disabled:opacity-60"
            disabled={playerData.gCoins < BIRTHDAY_CAKE_COST}
          >
            <CakeIcon className="w-5 h-5 mr-2" /> {UI_TEXT_TH.buyBirthdayCakeButtonLabel.replace('{cost}', BIRTHDAY_CAKE_COST.toString())}
          </button>
      ...
      {canDoFamilyActivity && (
        <div className="bg-slate-700/50 p-4 md:p-6 rounded-lg shadow-md mb-6">
          <h3 className="text-lg font-semibold mb-2 text-green-300 text-outline-black flex items-center">
            <UsersGroupIcon className="w-5 h-5 mr-2" /> {UI_TEXT_TH.familyActivityButtonLabel || "กิจกรรมครอบครัว"}
          </h3>
          <p className="text-xs text-slate-100 text-outline-black mb-3">
            {UI_TEXT_TH.familyActivityParkTripName || "ไปเที่ยวสวนสาธารณะ"} (ใช้ {FAMILY_ACTIVITY_PARK_TRIP_COST} G-Coins)
          </p>
          <button
            onClick={handlePerformParkTrip}
            disabled={!!parkTripCooldownDisplay || playerData.gCoins < FAMILY_ACTIVITY_PARK_TRIP_COST}
            className="w-full btn-success py-2.5 text-md flex items-center justify-center disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {parkTripCooldownDisplay
              ? (UI_TEXT_TH.familyActivityOnCooldownMessage?.replace('{itemName}', parkTripCooldownDisplay) || `รออีก ${parkTripCooldownDisplay}`)
              : (UI_TEXT_TH.familyActivityParkTripName || "ไปเที่ยวสวนสาธารณะ")}
          </button>
          {playerData.gCoins < FAMILY_ACTIVITY_PARK_TRIP_COST && !parkTripCooldownDisplay && (
             <p className="text-red-400 text-xs text-center mt-1">{UI_TEXT_TH.notEnoughGCoins}</p>
          )}
        </div>
      )}
      ...
      {child && (
          ...
          <div className="grid grid-cols-2 gap-2 text-xs mb-3">
            ...
            <p className="text-slate-100 text-outline-black">{UI_TEXT_TH.childSleepinessLabel} <span className="font-bold text-white">{child.sleepiness.toFixed(0)}/100</span></p>
             {child.growthStage === ChildGrowthStage.SCHOOL_AGE && (
              <>
                <p className="text-slate-100 text-outline-black">{UI_TEXT_TH.academicPerformanceLabel} <span className={`font-bold ${getAcademicPerformanceColor(child.academicPerformance)}`}>{child.academicPerformance}</span></p>
       ...
       <div className="bg-slate-700/50 p-4 md:p-6 rounded-lg shadow-md mb-6">
        ...
        <button
          ...
          disabled={!!buskingCooldownDisplay || !canPerformSideJob}
          ...
          title={!canPerformSideJob ? UI_TEXT_TH.sideJobFamilyUnhappyMessage : undefined}
        >
         ...
         {!canPerformSideJob && !buskingCooldownDisplay && (
            <p className="text-yellow-300 text-xs text-center mt-1">{UI_TEXT_TH.sideJobFamilyUnhappyMessage}</p>
        )}
      </div>
