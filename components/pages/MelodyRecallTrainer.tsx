...
import {
  ...
  getDifficultyText,
  ACTUAL_PIANO_LOWEST_MIDI_NOTE,
  ACTUAL_PIANO_HIGHEST_MIDI_NOTE,
  midiNoteToName,
  midiNoteToFrequency,
} from '../../constants';
...
const MelodyRecallTrainer: React.FC<MelodyRecallTrainerProps> = ({
  ...
  useEffect(() => {
    generateMelody();
  }, [difficulty]); // Removed generateMelody
  ...
  const handleNextQuestion = () => {
    generateMelody();
    if (highlightPianoOnPlay && !isCorrect) {
        setHighlightedNotesOnPiano([]);
    }
  };
  ...
  return (
    <GameContainer
      ...
      highlightedNotes={highlightedNotesOnPiano}
